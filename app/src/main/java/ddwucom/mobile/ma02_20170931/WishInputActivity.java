...
public class WishInputActivity extends AppCompatActivity {
    private static final String TAG = "WishInputActivity";

    final static int PERMISSION_REQ_CODE = 500;       // TODO: 외부저장소의 공용폴더에 저장할 때 사용할 것
    ...
    protected void onCreate(Bundle savedInstanceState){
        ...
        camera.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if (ActivityCompat.checkSelfPermission(WishInputActivity.this, Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
                    ActivityCompat.requestPermissions(WishInputActivity.this, new String[] {Manifest.permission.WRITE_EXTERNAL_STORAGE}, PERMISSION_REQ_CODE);
                }
                tedPermission();
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent intent) {
        ...
        if (requestCode == PICK_FROM_ALBUM) {
            ...
            Toast.makeText(WishInputActivity.this, "이미지를 불러오지 못했습니다.", Toast.LENGTH_SHORT).show();
        }
    }
        ...
        switch(v.getId()){
            case R.id.add_button:
                if(title.length() == 0) {
                    ...
                    value.put(WishDBHelper.COL_TITLE, title);
                    value.put(WishDBHelper.COL_AUTHOR, author);
                    value.put(WishDBHelper.COL_PUBLISHER, publisher);
                    value.put(WishDBHelper.COL_IMGTYPE, imgType);    // 갤러리:1 카메라:2
                    value.put(WishDBHelper.COL_IMGURL, String.valueOf(photoFile)); // 저장 경로

                    value.put(WishDBHelper.COL_STARTDAY, formatDate);

                    long count = db.insert(WishDBHelper.TABLE_NAME, null, value);
